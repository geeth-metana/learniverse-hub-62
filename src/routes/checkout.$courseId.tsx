...
function CheckoutPage() {
  ...
  const promoField = (
    <div className="w-full" style={{ maxWidth: 280 }} ref={promoWrapRef}>
      ...
      {promoError && (
        <p className="mt-1.5 text-smaller text-right" style={{ color: "#DC2626" }}>
          {promoError}
        </p>
      )}
    </div>
      ...
      <main className="min-h-screen flex flex-col items-center py-10 lg:py-16 px-6 lg:px-10">
        <div className="w-full max-w-6xl">
          <div className="flex items-center justify-between mb-8 gap-4 flex-wrap">
            ...
            <div
              className="inline-flex items-center bg-white rounded-full p-1"
              role="tablist"
          ...
          <div className="grid lg:grid-cols-2 gap-10 w-full">
            ...
            <div>
              <h1
                className="text-primary-header font-bold tracking-tight"
                style={{ color: TEXT_DARK }}
              >
              ...
              {!prefilled && !isSubscriptionInvite && (
                <>
                  <h3
                    className="text-second-header font-bold mt-8 mb-4"
                    style={{ color: TEXT_DARK }}
                  >
                    Choose Your Plan
                  </h3>
                    ...
                    {plans.map((p) => (
                      ...
                      >
                        ...
                        <div className="flex items-baseline gap-2">
                          <span
                            className="text-primary-header font-extrabold"
                            style={{ color: TEXT_DARK }}
                          >
                        ...
                        <p className="font-semibold text-small mt-1" style={{ color: TEXT_MUTED }}>
                          Upfront
                        </p>
                      </button>
                    ))}
              ...
              {isSubscriptionInvite ? (
                ...
                <div className="rounded-2xl bg-white p-5">
                  ...
                  <div className="flex items-start justify-between py-2 gap-4">
                    <span className="pt-2" style={{ color: TEXT_MUTED }}>
                      Promo Code
                    </span>
                    {promoField}
                  </div>
                    ...
                    <ProductDetailsList
                      rows={[
                        ...
                        { icon: Calendar, label: "Billing Cycle", value: "Monthly" },
                        {
                          ...
                          value: `$${subscriptionMonthly.toLocaleString()} / month`,
                        },
              ...
              {prefilled && !isSubscriptionInvite && (
                <>
                  {isBankInvite && invitation?.paymentDetails.paymentType === "Bank" && (
                    <>
                      <h3
                        className="text-second-header font-bold mt-8 mb-4 inline-flex items-center gap-2"
                        style={{ color: TEXT_DARK }}
                      >
                  ...
                  {isInstallmentInvite &&
                    invitation?.paymentDetails.paymentType === "Installment" && (
                      <>
                        <h3
                          className="text-second-header font-bold mt-8 mb-4"
                          style={{ color: TEXT_DARK }}
                          ...
                          <dl className="flex flex-col">
                            {(
                              [
                                ...
                                [
                                  "Selected Plan",
                                  invitation.paymentDetails.selectedPlan ?? "Installment Plan",
                                ],
                                [
                                  "Full Amount",
                                  `$${invitation.paymentDetails.fullAmount.toLocaleString()}`,
                                ],
                                ["Discount", `${invitation.paymentDetails.discountPercent ?? 0}%`],
                                [
                                  "Discounted Full Amount",
                                  `$${(invitation.paymentDetails.discountedFullAmount ?? invitation.paymentDetails.fullAmount).toLocaleString()}`,
                                ],
                                [
                                  "Down Payment",
                                  `$${invitation.paymentDetails.initialDownPayment.toLocaleString()}`,
                                ],
                                [
                                  "Monthly Payment",
                                  `$${Math.round(invitation.paymentDetails.monthlyPayment).toLocaleString()} / month`,
                                ],
                              ...
                              <div
                                ...
                                style={{
                                  borderBottom:
                                    i < arr.length - 1 ? "1px solid #F0F0F0" : undefined,
                                  backgroundColor:
                                    k === "Due Today" || k === "Down Payment"
                                      ? "rgba(204,246,33,0.18)"
                                      : undefined,
                                  paddingLeft:
                                    k === "Due Today" || k === "Down Payment" ? 12 : undefined,
                                  paddingRight:
                                    k === "Due Today" || k === "Down Payment" ? 12 : undefined,
                                  borderRadius:
                                    k === "Due Today" || k === "Down Payment" ? 8 : undefined,
                                }}
                              >
                                <dt style={{ color: TEXT_MUTED }}>{k}</dt>
                                <dd
                      ...
                      rows={[
                        {
                          icon: BookOpen,
                          label: "Course",
                          value: invitation?.course ?? course.title ?? "AI Builder Pack",
                        },
                        {
                          icon: Shield,
                          label: "Access Type",
                          value: invitation?.accessType ?? "Full Program Access",
                        },
                        {
                          icon: Calendar,
                          label: "Cohort Date",
                          value: invitation?.cohortDate ?? "Jun 12, 2026",
                        },
                        {
                          icon: CreditCard,
                          label: "Payment Type",
                        ...
                        { icon: Users, label: "Support", value: "Instructor-led guidance" },
                        {
                          ...
                          label: "Certificate",
                          value:
                            invitation?.certificateIncluded === false ? "Not included" : "Included",
                        },
                      ]}
                      muted={TEXT_MUTED}
                      dark={TEXT_DARK}
            ...
            <div className="h-fit flex flex-col gap-6">
              ...
              >
                <form onSubmit={submit} className="flex flex-col">
                  <h3 className="font-bold mb-4" style={{ color: TEXT_DARK }}>
                    Contact Info
                  </h3>
                  <label className="text-body block mb-2" style={{ color: TEXT_MAIN }}>
                    Billing Email
                  </label>
                  <input
                    type="email"
                  ...
                  <h3 className="font-bold mt-6 mb-4" style={{ color: TEXT_DARK }}>
                    Payment method
                  </h3>
                  {(() => {
                    const methods = [
                    ...
                    return (
                      <div
                        className="relative inline-flex w-full items-center rounded-full p-1"
                      ...
                      return (
                        <div className="mt-5 space-y-3">
                          ...
                          <div>
                            ...
                            >
                              <span
                                className="flex-1 truncate text-small font-mono"
                                style={{ color: TEXT_DARK }}
                              >
                                {walletAddress}
                              </span>
                          ...
                          <div>
                            ...
                            />
                            <p
                              className="text-smaller mt-1.5"
                              style={{ color: txError ? "#DC2626" : TEXT_MUTED }}
                            >
                              {txError
                    ...
                    (() => {
                      ...
                      return (
                        <div className="mt-5 space-y-4">
                          <div className="rounded-2xl p-4" style={{ backgroundColor: PAGE_BG }}>
                            ...
                            <dl className="bg-white rounded-xl px-4">
                              {(
                                ...
                                >
                                  <dt className="text-small" style={{ color: TEXT_MUTED }}>
                                    {k}
                                  </dt>
                                  <dd
                                    className="text-small font-semibold text-right break-all"
                                    style={{ color: TEXT_DARK }}
                                  >
                                    {v}
                                  </dd>
                          ...
                          <div>
                            <h4 className="font-semibold" style={{ color: TEXT_DARK }}>
                              {uploadLabel}
                            </h4>
                            ...
                            {!receipt ? (
                              ...
                              >
                                ...
                                <p className="text-smaller mt-1" style={{ color: TEXT_MUTED }}>
                                  or <span className="underline">Browse files</span> · PDF, PNG, JPG
                                  · Max 10MB
                                </p>
                                <input
                                  ref={fileInputRef}
                  ...
                  {isLoanInvite && invitation?.paymentDetails.paymentType === "Loan" ? (
                    ...
                    (() => {
                      ...
                      const disabled =
                        submitting || (isBank && !receipt) || (isCrypto && !txAddress.trim());
                      let label: string;
                  ...
                  >
                    ...
                    <span className="font-bold" style={{ color: TEXT_DARK }}>
                      stripe
                    </span>
                  </div>
                  <div
                    className="mt-1 flex items-center justify-center gap-4 text-small"
                    style={{ color: TEXT_MUTED }}
                  >
                    <span>Legal</span>
                    <span>Returns</span>
      ...
      {rows.map((r, i) => {
        ...
        return (
          ...
          >
            ...
            <dd className="font-semibold text-right" style={{ color: dark }}>
              {r.value}
            </dd>
          </div>
        );
